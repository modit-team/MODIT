...
def read_examples(filename):
    ...
    trg_filename = filename.split(',')[2]
    idx = 0
    with open(src1_filename) as f1, open(src2_filename) as f2, open(trg_filename) as f3:
...
def convert_examples_to_features(examples, tokenizer, args, stage=None):
    ...
    for example_index, example in enumerate(tqdm(examples)):
        # source1
        source_tokens1 = tokenizer.tokenize(example.source1)[:args.max_source_length - 2]
        ...
        padding_length1 = args.max_source_length - len(source_ids1)
        source_ids1 += [tokenizer.pad_token_id] * padding_length1
        source_mask1 += [0] * padding_length1

        # source2
        source_tokens2 = tokenizer.tokenize(example.source2)[:(args.max_source_length - 2)]
        ...
        padding_length2 = args.max_source_length - len(source_ids2)
...
def main():
    ...
    model = DualEncoderModel(encoder1=encoder1, encoder2=encoder2, decoder=decoder, config=config,
    ...
    if args.do_train:
        ...
        train_data = TensorDataset(all_source_ids1, all_source_mask1, all_source_ids2, all_source_mask2, all_target_ids,
                                   all_target_mask)
        if args.local_rank == -1:
        ...
        for _ in tqdm(bar):
            ...
            source_ids1, source_mask1, source_ids2, source_mask2, target_ids, target_mask = batch
            ...
            if args.do_eval and ((global_step + 1) % args.eval_steps == 0) and eval_flag:
                ...
                for batch in tqdm(eval_dataloader):
                    batch = tuple(t.to(device) for t in batch)
                    source_ids1_eval, source_mask1_eval, source_ids2_eval, source_maks2_eval, target_ids_eval, target_mask_eval = batch
                ...
                for batch in tqdm(eval_dataloader):
                    batch = tuple(t.to(device) for t in batch)
                    source_ids1_dev, source_mask1_dev, source_ids2_dev, source_mask2_dev = batch
                    with torch.no_grad():
    ...
    if args.do_test:
        ...
        for idx, file in enumerate(files):
            ...
            for batch in tqdm(eval_dataloader, total=len(eval_dataloader)):
                batch = tuple(t.to(device) for t in batch)
                source_ids1, source_mask1, source_ids2, source_mask2 = batch
                with torch.no_grad():
