...
def eval_bleu(args, model, tokenizer, file_type='test', num=99999999):
    ...
    model.eval()
    preds = []
    for step, (batch, token_labels) in enumerate(tqdm(test_dataloader, total=min(num, len(dataset)))):
        if step >= num:
            break
        inputs = batch.to(args.device)
        with torch.no_grad():
...
def main():
    ...
    if args.do_eval:  # only works on 1 GPU
        dev_bleu, dev_EM = eval_bleu(args, model, tokenizer, file_type='eval')
        logger.info(f"dev bleu: {dev_bleu}, dev EM: {dev_EM}")
        print(f"dev bleu: {dev_bleu}, dev EM: {dev_EM}")

    if args.do_infer:  # only works on 1 GPU
        test_bleu, test_EM = eval_bleu(args, model, tokenizer, file_type='test')
        logger.info(f"test bleu: {test_bleu}, test EM: {test_EM}")
        print(f"test bleu: {test_bleu}, test EM: {test_EM}")
